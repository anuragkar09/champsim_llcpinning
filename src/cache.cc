...
void CACHE::handle_fill()
{
  while (writes_available_this_cycle > 0) {
    ...
    if (way != max_way) { // Found a non-reserved way
      ...
      //++anurag
      uint64_t ch, ra, ba, ro;
      ro = dram_get_row(fill_mshr->address);
      ba = dram_get_bank(fill_mshr->address);
      ra = dram_get_rank(fill_mshr->address);
      ch = dram_get_channel(fill_mshr->address);
      ...
      //print CRA_idx
      //std::cout << "CRA_idx: " << CRA_idx << std::endl;
      //print DRAM_CHANNELS*DRAM_BANKS*DRAM_RANKS*DRAM_ROWS
      //std::cout << "DRAM_CHANNELS*DRAM_BANKS*DRAM_RANKS*DRAM_ROWS: " << DRAM_CHANNELS*DRAM_BANKS*DRAM_RANKS*DRAM_ROWS << std::endl;
      //std::cout << "block[set * NUM_WAY + way].data: " << block[set * NUM_WAY + way].data << std::endl;
      //print CRA_ctr[CRA_idx]
      //std::cout << "CRA_ctr[CRA_idx]: " << CRA_ctr[CRA_idx] << std::endl;
      ...
      if(CRA_ctr !=NULL)
      {
        if (CRA_ctr[CRA_idx] > (RH_THRESHOLD/2))
        {
          ...
          //std::cout << "block[set * NUM_WAY + way].data: " << block[set * NUM_WAY + way].data << std::endl;
          block[set * NUM_WAY + way].pinned = true;
          //std::cout << "Pinning the block" << std::endl;
        }
      }
      //++anurag end 
...
void CACHE::operate_reads()
{
  ...
  if (NAME == "LLC") {
    if (current_cycle - last_tracker_reset >= 256000000ul) {
      ...
      //++anurag unpinning all pinned rows
      for(int i = 0; i < NUM_SET; i++)
      {
        for(int j = 0; j < NUM_WAY; j++)
        {
          if(block[i*NUM_WAY + j].pinned)
          {
            //std::cout << "Unpinning the block" << std::endl;
          }
          block[i*NUM_WAY + j].pinned = false;
        }
      }
...
uint64_t CACHE::dram_get_channel(uint64_t address)
{
  int shift = LOG2_BLOCK_SIZE + lg2(DRAM_COLUMNS);
  return (address >> shift) & bitmask(lg2(DRAM_CHANNELS));
}

uint64_t CACHE::dram_get_bank(uint64_t address)
{
  int shift = lg2(DRAM_CHANNELS) + lg2(DRAM_COLUMNS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DRAM_BANKS));
}

uint64_t CACHE::dram_get_rank(uint64_t address)
{
  int shift = lg2(DRAM_BANKS) + lg2(DRAM_CHANNELS) + lg2(DRAM_COLUMNS) + LOG2_BLOCK_SIZE;
  return (address >> shift) & bitmask(lg2(DRAM_RANKS));
}

uint64_t CACHE::dram_get_row(uint64_t address)
{
  int shift = lg2(DRAM_RANKS) + lg2(DRAM_BANKS) + lg2(DRAM_CHANNELS) + lg2(DRAM_COLUMNS) + LOG2_BLOCK_SIZE;
